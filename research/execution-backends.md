...
- Live run observed: `hcn run pi --json "Reply with exactly one word: ok"` from /tmp, exit 0, cause `clean`. Event kinds present: `identity` (x2, announce + attestation), `message`, `token`, `done`. **Observed** - capture at `research/captures/pi-run-ok.ndjson` on this branch.
